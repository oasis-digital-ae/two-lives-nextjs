/* -------------------------------------------------------
 ...
 Theme Name: Crafto - The Multipurpose HTML5 Templatef
...
(function ($) {
    ...
    $(document).on('click', 'body', function (e) {
        var clickedOnNavbarControl = $(e.target).closest('.navbar-toggler, .navbar-collapse, .dropdown-toggle').length;
        // Close all menu
        if (!clickedOnNavbarControl && !($(e.target).closest('.navbar-nav').length || $(e.target).closest('.navbar-full-screen-menu-inner').length)) {
            setTimeout(function () {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const subtitleBox = document.getElementById("customSubtitles");
  ...
  let activeTrack = null;
    const miniParent = container.parentNode;
    const miniNextSibling = container.nextSibling;

    const attachMiniPlayer = () => {
        if (container.parentNode !== document.body) {
            document.body.appendChild(container);
        }
    };

    const detachMiniPlayer = () => {
        if (miniParent && container.parentNode === document.body) {
            miniParent.insertBefore(container, miniNextSibling);
        }
    };
  ...
  function activateTrack(lang) {
    ...
    // Keep browser hidden (custom rendering)
    ...
    activeTrack.oncuechange = () => {
      ...
      if (cue) {
        subtitleBox.innerHTML = cue.text;
    ...
    // 🔥 Immediate refresh (important)
    if (ccEnabled && activeTrack.activeCues && activeTrack.activeCues.length > 0) {
      const cue = activeTrack.activeCues[0];
      subtitleBox.innerHTML = cue.text;
      subtitleBox.classList.add("active");
    }
  ...
  ccBtn.addEventListener("click", () => {
    ...
    if (ccEnabled) {
      ...
      // Re-activate current language
      activateTrack(subtitleSelect.value);
  ...
  playBtn.addEventListener("click", () => {
    ...
    // 🔥 Ensure subtitle sync when playback starts
    if (ccEnabled) {
      activateTrack(subtitleSelect.value);
    }
  });
  ...
  video.addEventListener("timeupdate", () => {
    ...
    const tot = fmt(video.duration || 0);
  ...
  window.addEventListener("scroll", () => {
    ...
    if (!isPlaying || !userHasPlayedVideo || manuallyClosed) return;

    const heroBottom = hero.getBoundingClientRect().bottom;

    if (heroBottom < -80 && !miniActive) {
            attachMiniPlayer();
      container.classList.add("mini");
      closeMini.classList.remove("hidden");
      miniActive = true;
    }

    if (heroBottom >= 0 && miniActive) {
      container.classList.remove("mini");
      closeMini.classList.add("hidden");
            detachMiniPlayer();
      miniActive = false;
    }
  });
...
// Floating widgets adjustment for video mini mode
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const widgets = document.querySelectorAll(
        ".whatsapp-widget, .micro-consult-trigger"
    );

    if (!videoContainer || !widgets.length) return;

    const updateWidgets = () => {
        if (videoContainer.classList.contains("mini")) {
            widgets.forEach(w => w.classList.add("raise-for-video"));
        } else {
            widgets.forEach(w => w.classList.remove("raise-for-video"));
        }
    };

    // Watch class changes on video container
...
document.addEventListener("DOMContentLoaded", function () {
  const trigger = document.querySelector(".micro-consult-trigger");
  const consultButtons = document.querySelectorAll(".consult");
  const modal = document.getElementById("microConsultModal");
  const closeBtn = document.querySelector(".micro-close");
  const overlay = document.querySelector(".micro-consult-overlay");
  ...
  // Open modal from consult buttons
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    .then(() => {
      ...
      setTimeout(() => {
        if (modal) {
          modal.classList.remove("active"); // if you use active class
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const skeletonContainer = document.getElementById("case-skeletons");
    const realGrid = document.getElementById("case-grid");

    if (!skeletonContainer || !realGrid) return;
...
// Reviews Paginaation nd autoplay
document.addEventListener("DOMContentLoaded", function () {

    const testimonialImages = [
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/grant.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/Cattaleeya.jpg",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/punnat.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/dan-moores.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/lauri-enn.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/ellie-jackson.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/brayden-clarke.png",
        "https://two-lives-theory.netlify.app/.netlify/images?url=/images/tom-russell.png"
    ];

    new Swiper(".testimonials-style-06", {
        loop: true,
        ...
        pagination: {
            ...
            renderBullet: function (index, className) {
                return '<span class="' + className + '">' +
                       '<img src="' + testimonialImages[index] + '">' +
                       '</span>';
            }
        }
    });
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    new Swiper(".slider-one-slide", {
        ...
        pagination: {
            el: ".slider-one-slide-pagination",
            clickable: true
        },
...
// {
// name:"Entrepreneurs",
// description:`Builders who have achieved success but feel there is another level calling them. I mentor entrepreneurs to strengthen their mindset, steady their emotions, and align their work with their life so growth feels powerful, not heavy.`,
// link:"/entrepreneur",
// background:"https://two-lives-theory.netlify.app/.netlify/images?url=/images/two-lives/entreprenuer.png"
// },

// {
// name:"Athletes",
// description:`Competitors who understand that performance starts from within. I work with athletes to build mental strength, emotional control, and self belief so they grow not only in sport, but in who they are becoming off the field as well.`,
// link:"/athlete",
// background:"https://two-lives-theory.netlify.app/.netlify/images?url=/images/two-lives/athlete.png"
// },

// {
// name:"Elite Competitors",
// description:`High level individuals operating where standards are relentless. I help elite competitors manage pressure, sharpen focus, and evolve their identity so success is sustainable and the next chapter feels intentional, not accidental.`,
// link:"/elite-competitors",
// background:"https://two-lives-theory.netlify.app/.netlify/images?url=/images/two-lives/poker-player.jpg"
// },

// {
// name:"Executives & Leaders",
// description:`Leaders carrying pressure, responsibility, and constant decisions. I mentor executives to think clearly, stay composed, and build a strong inner foundation so their leadership feels calm, decisive, and balanced across work and home.`,
// link:"/executive-leaders",
// background:"https://two-lives-theory.netlify.app/.netlify/images?url=/images/two-lives/exec-leader.png"
// }
...
// li.style.backgroundImage=`url(${artist.background})`;

// if(index===0) li.classList.add("active");

// li.innerHTML=`

// <h3 class="alt-font fw-500 ls-minus-2px fs-40 mb-3 md-mb-10">${artist.name}</h3>
...
// <h2 class="alt-font fw-500 ls-minus-2px fs-40 mb-3">${artist.name}</h2>

// <p>${artist.description}</p>

// <a href="${artist.link}" class="btn-primary active btn-box-shadow fw-600 mentor-btn">
// Learn More
// <span class="text-carbon">
// <i class="pl-10px feather icon-feather-arrow-right"></i>
// </span>
// </a>
...
// document.querySelectorAll("#artistsList li").forEach(el=>{
...
document.querySelectorAll('#track').forEach(track => {
  ...
  const dotsBox = section.querySelector('#dots');
  ...
  if (dotsBox && window.matchMedia("(max-width:767px)").matches) {
    dotsBox.hidden = true;
  }
...
// counter on scroll
const counters = document.querySelectorAll('.counter-percent');
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const existingInstance = slider.swiper;
    if (existingInstance) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const elements = document.querySelectorAll(".mentor-animate-words");

    elements.forEach(el => {

        const text = el.textContent.trim();
        const words = text.split(" ");
        const highlightWord = (el.getAttribute("data-highlight-word") || "").trim();
        const highlightPhrase = (el.getAttribute("data-highlight-phrase") || "").trim();
        const highlightIndexAttr = el.getAttribute("data-highlight-index");
        const highlightIndex = highlightIndexAttr !== null && highlightIndexAttr !== "" ? parseInt(highlightIndexAttr, 10) : NaN;
        const normalizeWord = value => value.replace(/^[^\w]+|[^\w]+$/g, "").toLowerCase();
        const normalizePhrase = value => value.split(/\s+/).map(normalizeWord).filter(Boolean).join(" ");
        const phraseWords = highlightPhrase ? highlightPhrase.split(/\s+/).filter(Boolean) : [];
        const targetIndex = Number.isInteger(highlightIndex)
            ? highlightIndex
            : (highlightPhrase
                ? words.findIndex((word, index) => {
                    const candidate = words.slice(index, index + phraseWords.length).map(normalizeWord).join(" ");
                    return candidate === normalizePhrase(highlightPhrase);
                })
                : (highlightWord ? words.findIndex(word => normalizeWord(word) === normalizeWord(highlightWord)) : 0));
        const resolvedTargetIndex = targetIndex >= 0 ? targetIndex : 0;
        const highlightSpanLength = highlightPhrase && targetIndex >= 0 ? phraseWords.length : 1;
        el.innerHTML = "";

        for (let i = 0; i < words.length; i++) {
            const isTargetStart = i === resolvedTargetIndex;
            const isPhraseTarget = highlightSpanLength > 1 && isTargetStart;
            const wordText = isPhraseTarget ? words.slice(i, i + highlightSpanLength).join(" ") : words[i];

            const span = document.createElement("span");
            span.className = "word";
            span.textContent = wordText;
            span.style.transitionDelay = (i * 180) + "ms";

            // Highlight the requested word, or fall back to the first word.
            if (isTargetStart) {
                span.classList.add("highlighter-below", "animate", "mb-0", "pb-0");
                span.setAttribute("data-shadow-animation", "true");
                span.setAttribute("data-animation-delay", "500");

                const highlight = document.createElement("span");
                highlight.className = "highlighter-below animate";
                span.appendChild(highlight);

                if (isPhraseTarget) {
                    i += (highlightSpanLength - 1);
                }
            }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const btn = document.querySelector(".desktop-toggler");
    ...
    // observe header class changes (sticky detection)
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const widgets = document.querySelectorAll(".whatsapp-widget, .micro-consult-trigger");
...
(function () {
  ...
  function removeBlur() {
    ...
    const style = document.getElementById("calendly-blur-style");
  ...
  function check() {
    ...
    const visible = isVisible(popup) || isVisible(overlay);

    if (visible) applyBlur();
    else removeBlur();
  }
